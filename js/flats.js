...
function RandomNote () {
    ...
    var randomNum = Math.floor(Math.random() * flats.length); // change here

    // no repeat notes - check if current number = either of last 2 numbers
    while (randomNum == lastNums[0] || randomNum == lastNums[1]) {
        randomNum = Math.floor(Math.random() * flats.length); // change here
    }

    var randomNote = flats[randomNum]; // change here
